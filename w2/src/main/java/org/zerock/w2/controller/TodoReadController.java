...
public class TodoReadController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
             ...
             * 이때 브라우저가 보낸 쿠키 중에 "viewTodos"라는 key값을 가진 쿠키가 존재하는지 찾아본다.
             */
            // 쿠키 찾기
            ...
            // 기존 쿠키의 value가 존재하고
            // 현재 jsp에서 보여주는 tno에 이전에 접근한 적이 있다면(즉, 기록이 남아 있다면)
            if(todoListStr != null && todoListStr.indexOf(tno + "-") >= 0) {
                exist = true;       // 이전의 기록이 존재함
            }
    ...
    // 우리가 원하는 쿠키를 브라우저가 전송한 쿠키들에서 찾아 쓰면 반환하고,
    // 못 찾았으면 새로 만들어서 반환함
